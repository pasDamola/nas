      ...
      <div class="row justify-content-between align-items-center">
        <div class="col-12 col-md-6 login__hero">
          <span class="my-2 d-block">
            ...
            <a href="/supervisors/apply">Apply here!</a>
          </span>
...
export default {
  ...
  methods: {
    logIn () {
      this.loading = true
      this.$axios.post('supervisor', this.user).then((res) => {
        this.loading = false
        console.log(res)
        // if (this.saveDetails) {
        ...
        //   this.$store.commit('SET_VENDOR_DETAILS', res.data.user)
        // }
        sessionStorage.setItem('nasims-supervisor', JSON.stringify(res.data.user))
        const id = window.btoa(res.data.user.email)
        this.$router.push(`/supervisors/${id}`)
      }).catch((err) => {
        this.loading = false
